...
async function convertMdxToMarkdown(
  ...
  const { includeSourceCode = true, includeInstallInstructions = true } =
    options

  try {
    // Read the MDX file
    const filePath =
      path.join(baseDir, CONTENT_DIRECTORY, slug.join("/")) + ".mdx"
    ...
    convertedContent = convertedContent.replace(
      ...
      (match: string, name: string) => {
        try {
          ...
          return `See the interactive demo at: [${name}](https://sesameui.vercel.app/docs/components/${slug.join("/")})\n\n`
        } catch (error) {
    ...
    convertedContent = convertComponentSourceToMarkdown(
      convertedContent,
      includeSourceCode
    )

    // Convert InstallTabs to markdown
    convertedContent = convertInstallTabsToMarkdown(
      convertedContent,
    ...
    // Add footer
    markdown += `\n\n---\n\n*This documentation is also available in [interactive format](https://sesameui.vercel.app/docs/components/${slug.join("/")}).*\n`
...
function convertLinksToMarkdown(content: string): string {
  ...
  return content.replace(
    ...
    (match, href, linkText) => {
      ...
      let fullUrl = href
      if (href.startsWith("/")) {
        fullUrl = `https://sesameui.vercel.app${href}.md`
      }
...
function convertInstallTabsToMarkdown(
  content: string,
...
function convertTablesToMarkdown(content: string): string {
  ...
  result = result.replace(/<Table[^>]*>[\s\S]*?<\/Table>/g, (tableBlock) => {
    ...
    const headerMatch = tableBlock.match(
      /<TableHeader[^>]*>([\s\S]*?)<\/TableHeader>/
    )
    const bodyMatch = tableBlock.match(
      /<TableBody[^>]*>([\s\S]*?)<\/TableBody>/
    )
...
function convertTabsToMarkdown(content: string): string {
  ...
  result = result.replace(/<Tabs[^>]*>[\s\S]*?<\/Tabs>/g, (tabsBlock) => {
    ...
    const tabsContentMatches = tabsBlock.match(
      /<TabsContent[^>]*value="([^"]+)"[^>]*>([\s\S]*?)<\/TabsContent>/g
    )

    if (!tabsContentMatches) return ""

    return tabsContentMatches
      .map((tabMatch) => {
        const valueMatch = tabMatch.match(/value="([^"]+)"/)
        const contentMatch = tabMatch.match(
          /<TabsContent[^>]*>([\s\S]*?)<\/TabsContent>/
        )
