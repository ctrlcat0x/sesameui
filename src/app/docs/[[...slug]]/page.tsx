...
export default async function DocPage({ params }: DocPageProps) {
  ...
  // Use the resolved doc.slug (returned by getDocFromParams) instead of
  // accessing `params.slug` directly — Next.js v15 requires awaiting params
  const slugArray = doc.slug ? String(doc.slug).split("/") : []

  const componentType = slugArray[1]
    ? slugArray[1].charAt(0).toUpperCase() + slugArray[1].slice(1).toLowerCase()
    : "Getting Started"
