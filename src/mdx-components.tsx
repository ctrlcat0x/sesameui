...
export function mdxComponents(components?: MDXComponents): MDXComponents {
  return {
    ...
    img: ({
      ...
      // Ensure we don't pass an empty string to src — normalize to null instead
      const rawSrc = props.src as unknown as string | undefined | null
      const safeSrc =
        rawSrc && typeof rawSrc === "string" && rawSrc.trim() !== ""
          ? rawSrc
          : null
    ...
    TableBody: ({
      ...
      <TableBody className={cn("font-mono text-xs", className)} {...props} />
    ),
    TableRow: ({
      className,
      ...props
    ...
    TableCell: ({
      ...
      <TableCell
        className={cn(
          "font-mono text-xs [&_*]:!text-xs [&_code]:!text-xs [&_code]:whitespace-pre-wrap [&_code]:box-decoration-clone",
          className
        )}
        {...props}
      />
